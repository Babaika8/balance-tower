...
## Рабочее правило пользователя

После завершённой и локально проверенной правки игры автоматически запускай
живой деплой. Пользователь оценивает изменения непосредственно в Telegram, а не
по локальным промежуточным материалам. Не деплой сломанную или непроверенную
сборку.

Используй только один продуктовый контур: исходники в `main`, публикация в корень
`https://babaika8.github.io/balance-tower/`, этот же адрес открывает Telegram.
Не создавай альтернативные игровые рантаймы, `/next/` или другие параллельные
пути. Blender используется для производства графики и анимации существующей
игры; физика, реклама, рейтинг, бонусы и переключение сцен сохраняются.
