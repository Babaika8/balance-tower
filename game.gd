...
const BUILD_TAG := "frozen-3"   # ВРЕМЕННО: метка сборки + HUD позиций
...
func _continue_game() -> void:
	...
	for e in stable_snapshot:
		...
		if is_instance_valid(s):
			...
			s.angular_velocity = 0.0
			# ВАЖЕН ПОРЯДОК: сначала замораживаем (kinematic), ПОТОМ ставим позицию —
			# иначе при заморозке тело берёт координату из физсервера (место падения) и
			# восстановленные блоки «схлопываются» за экран. Заморозка делает башню
			# статичной опорой: живые тела на стыках рассыпались бы, а так держится намертво.
