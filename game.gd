...
const BUILD_TAG := "frozen-4"   # ВРЕМЕННО: метка сборки + HUD позиций
...
func _process(delta: float) -> void:
	...
	if dbg_label:
		...
		dbg_label.text = "%s st=%d stones=%d/%d topY=%.0f camY=%.0f stoneTopY=%.0f" % [
			...
			camera.position.y if camera else 0.0, sy]
...
func _physics_process(_delta: float) -> void:
	...
	for s in stones:
		...
		if absf(s.rotation - s.get_meta("rot_base", 0.0)) > COLLAPSE_ANGLE or s.global_position.y > miny + COLLAPSE_DROP:
			...
			return
	# Снимок для «Продолжить» снимаем ТОЛЬКО когда башня спокойна — иначе замораживаем
	# подпрыгнувшие блоки и башня восстанавливается с щелями.
	if state == State.WAITING and score > 0:
		var calm := true
		for s in stones:
			if is_instance_valid(s) and s.get_meta("placed", false) and not s.get_meta("frozen_base", false):
				if s.linear_velocity.length() > 12.0 or absf(s.angular_velocity) > 0.15:
					calm = false
					break
		if calm:
...
func _drop() -> void:
	# Снимок для «Продолжить» снимается в _physics_process в момент покоя башни,
	# а не здесь (в момент броска башня часто ещё качается → щели после заморозки).
...
func _continue_game() -> void:
	...
	for s in stones.duplicate():
		if not keep.has(s):
			if is_instance_valid(s):
				s.queue_free()
			stones.erase(s)
	# Пересобираем башню ПЛОТНОЙ стопкой — детерминированно, без вертикальных щелей.
	# Сохраняем X и наклон каждого блока (характер башни), но вертикальный шаг делаем
	# равномерным и тесным. Замораживаем (kinematic) как статичную опору: живые тела на
	# стыках рассыпались бы, а так последняя устойчивая комбинация держится намертво.
	...
	var col: Array = []
	for e in stable_snapshot:
		if is_instance_valid(e["stone"]):
			col.append(e)
	col.sort_custom(func(a, b): return a["pos"].y > b["pos"].y)   # снизу (большой y) вверх
	var step := ssize.y * 0.8
	var base_y: float = col[0]["pos"].y if col.size() > 0 else ground_top_y - ssize.y * 0.5
	for i in range(col.size()):
		var s: RigidBody2D = col[i]["stone"]
		s.linear_velocity = Vector2.ZERO
		s.angular_velocity = 0.0
		s.freeze_mode = RigidBody2D.FREEZE_MODE_KINEMATIC
		s.freeze = true
		s.global_position = Vector2(base_x, base_y - i * step)
		s.rotation = 0.0
		s.set_meta("placed", true)
		s.set_meta("frozen_base", true)      # детектор обвала игнорирует этот блок
	score = stable_score
	if col.size() > 0:
		top_y = (base_y - (col.size() - 1) * step) - ssize.y * 0.5
		last_placed = col[col.size() - 1]["stone"]
	else:
		top_y = stable_top_y
		last_placed = null
	current_stone = null
