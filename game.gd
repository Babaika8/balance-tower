...
var _zen_stone_texs: Array = []              # PNG-текстуры камней Дзен (3 вида, если есть)
var _zen_stone_texs_loaded := false
...
var zen_life_waterfalls: Array = [] # {node, x, y, phase, alpha, len}
...
func _setup_zen_life_overlay() -> void:
	...
	for i in range(7):
		...
		zen_life_fog.append({"node": fog, "nx": randf(), "ny": [155.0, 260.0, 390.0, 515.0, 650.0, 780.0, 910.0][i],
				"speed": randf_range(0.010, 0.024), "alpha": randf_range(0.055, 0.125)})
	for pos in [[64.0, 760.0], [92.0, 850.0], [112.0, 940.0], [610.0, 710.0], [648.0, 825.0], [660.0, 928.0], [92.0, 1060.0], [618.0, 1080.0],
			[544.0, 790.0], [578.0, 868.0], [138.0, 900.0], [176.0, 1032.0], [602.0, 1018.0]]:
		...
		zen_life_water.append({"node": glint, "x": pos[0], "y": pos[1], "phase": randf() * TAU,
				"alpha": randf_range(0.24, 0.52)})
	for pos in [[382.0, 565.0, 78.0, 0.18], [616.0, 720.0, 86.0, 0.20], [102.0, 820.0, 70.0, 0.14]]:
		var wf := _make_waterfall_stream(float(pos[2]))
		zen_life_layer.add_child(wf)
		zen_life_waterfalls.append({"node": wf, "x": pos[0], "y": pos[1], "phase": randf() * TAU,
				"alpha": pos[3], "len": pos[2]})
	...
	for i in range(7):
		var cl := _make_cloud()
		cl.scale = Vector2(randf_range(0.85, 1.55), randf_range(0.65, 1.12))
		zen_life_layer.add_child(cl)
		zen_life_clouds.append({"node": cl, "nx": randf(), "ny": randf_range(65.0, 420.0),
				"speed": randf_range(0.006, 0.014), "alpha": randf_range(0.11, 0.22)})
	for i in range(34):
...
func _update_zen_life() -> void:
	...
	for c in zen_life_clouds:
		...
		var cn: Node2D = c["node"]
		cn.position = Vector2(lerp(-320.0, vw + 320.0, float(c["nx"])),
				float(c["ny"]) + sin(t * 0.22 + float(c["nx"]) * TAU) * 8.0)
		cn.modulate = Color(1, 1, 1, lerp(float(c["alpha"]) * 0.85, float(c["alpha"]) * 1.9, sky_phase))
	...
	for fband in zen_life_fog:
		...
		fn.position = Vector2(lerp(-300.0, 300.0, float(fband["nx"])),
				float(fband["ny"]) + sin(t * 0.38 + float(fband["nx"]) * TAU) * 10.0)
		fn.modulate.a = float(fband["alpha"]) * (0.72 + 0.28 * sin(t * 0.55 + float(fband["nx"]) * TAU)) * lerp(1.0, 0.55, sky_phase)
	for w in zen_life_water:
		var wn: Line2D = w["node"]
		wn.position = Vector2(float(w["x"]) + sin(t * 1.5 + float(w["phase"])) * 16.0,
				float(w["y"]) + sin(t * 2.2 + float(w["phase"]) * 0.7) * 3.0)
		wn.modulate.a = float(w["alpha"]) * (0.35 + 0.65 * (0.5 + 0.5 * sin(t * 3.4 + float(w["phase"]))))
	for wf in zen_life_waterfalls:
		var wfn: Node2D = wf["node"]
		wfn.position = Vector2(float(wf["x"]) + sin(t * 0.7 + float(wf["phase"])) * 2.0, float(wf["y"]))
		wfn.modulate.a = float(wf["alpha"]) * (0.62 + 0.38 * sin(t * 1.4 + float(wf["phase"])))
		var idx := 0
		for child in wfn.get_children():
			if child is Line2D:
				var line := child as Line2D
				line.position.y = fposmod(t * (22.0 + idx * 7.0) + float(wf["phase"]) * 8.0 + idx * 12.0, float(wf["len"])) - float(wf["len"]) * 0.5
				idx += 1
...
func _make_fog_band() -> Line2D:
	var l := Line2D.new()
	l.width = randf_range(10.0, 22.0)
	l.default_color = Color(0.86, 0.94, 1.0, 1.0)
	l.points = PackedVector2Array([
		Vector2(-180, 0), Vector2(50, -8), Vector2(280, 4), Vector2(540, -7), Vector2(900, 3)
	])
...
func _make_water_glint() -> Line2D:
	var l := Line2D.new()
	l.width = randf_range(2.5, 5.0)
	l.default_color = Color(0.72, 0.95, 1.0, 1.0)
	var w := randf_range(34.0, 86.0)
	l.points = PackedVector2Array([Vector2(-w * 0.5, 0), Vector2(w * 0.5, randf_range(-2.0, 2.0))])
	l.begin_cap_mode = Line2D.LINE_CAP_ROUND
	l.end_cap_mode = Line2D.LINE_CAP_ROUND
	return l

func _make_waterfall_stream(len: float) -> Node2D:
	var n := Node2D.new()
	n.z_index = -64
	for i in range(5):
		var l := Line2D.new()
		l.width = [2.0, 3.0, 2.0, 4.0, 2.0][i]
		l.default_color = Color(0.78, 0.94, 1.0, 1.0)
		var x := float(i - 2) * 5.0 + randf_range(-2.0, 2.0)
		var h := len * randf_range(0.28, 0.58)
		l.points = PackedVector2Array([Vector2(x, -h * 0.5), Vector2(x + randf_range(-2.0, 2.0), h * 0.5)])
		l.begin_cap_mode = Line2D.LINE_CAP_ROUND
		l.end_cap_mode = Line2D.LINE_CAP_ROUND
		l.modulate.a = randf_range(0.45, 0.9)
		n.add_child(l)
	return n
...
uniform float sway_strength = 0.0105;
...
float foliage_mask(vec4 c) {
	...
	float sat = mx - mn;
	float green = smoothstep(0.005, 0.105, c.g - max(c.r * 0.82, c.b * 0.66));
...
func _process(delta: float) -> void:
	...
	if dbg_label:
		...
		for s in stones:
			if is_instance_valid(s):
				nvalid += 1
				sy = min(sy, s.global_position.y)
		dbg_label.text = "%s st=%d stones=%d/%d topY=%.0f camY=%.0f stoneTopY=%.0f\n%s | %s" % [
			BUILD_TAG, state, nvalid, stones.size(), top_y,
			camera.position.y if camera else 0.0, sy, _dbg_cont, _dbg_snap]
...
func _physics_process(_delta: float) -> void:
	...
	for s in stones:
		if not is_instance_valid(s) or not s.get_meta("placed", false) or s.get_meta("frozen_base", false):
			continue
		...
		if absf(s.rotation - s.get_meta("rot_base", 0.0)) > COLLAPSE_ANGLE or s.global_position.y > miny + COLLAPSE_DROP:
			...
			return
func _capture_stable_snapshot() -> void:
	...
	_dbg_snap = "snap=%d sScore=%d sTop=%.0f" % [stable_snapshot.size(), stable_score, stable_top_y]
...
func _drop() -> void:
	# Снимок последней башни (до этого броска) — для «Продолжить».
	_capture_stable_snapshot()
	...
	for s in stones.duplicate():
		if not keep.has(s):
			if is_instance_valid(s):
				s.queue_free()
			stones.erase(s)
	# Восстанавливаем блоки на их сохранённые места и замораживаем как статичную опору.
	# ВАЖЕН ПОРЯДОК: сперва freeze=true, потом позиция — иначе при заморозке тело берёт
	# координату из физсервера (место падения) и блок «схлопывается» за экран.
	var nrestored := 0
	for e in stable_snapshot:
		...
		if is_instance_valid(s):
			...
			s.angular_velocity = 0.0
			# KINEMATIC телепортирует блок визуально верно. Но физическую коллизию
			# нужно задать ПРИНУДИТЕЛЬНО через физсервер (без «свипа»), иначе остаётся
			# «призрак» коллизии в месте падения и новый блок зависает с зазором.
			s.freeze_mode = RigidBody2D.FREEZE_MODE_KINEMATIC
			s.freeze = true
			# Ровная колонна: по центру (base_x) и прямо (без наклона), высоту сохраняем.
			var xf := Transform2D(0.0, Vector2(base_x, e["pos"].y))
			s.global_transform = xf
			PhysicsServer2D.body_set_state(s.get_rid(), PhysicsServer2D.BODY_STATE_TRANSFORM, xf)
			s.set_meta("placed", true)
			s.set_meta("frozen_base", true)
			nrestored += 1
	# top_y берём по ФАКТИЧЕСКОЙ вершине восстановленных блоков, а не из сохранённого
	# stable_top_y (он рассинхронизируется) — иначе рука встаёт не на ту высоту и новый
	# блок ложится с разрывом.
	score = stable_score
	var top_block: RigidBody2D = null
	var min_top := INF
	for e2 in stable_snapshot:
		var s2: RigidBody2D = e2["stone"]
		if is_instance_valid(s2):
			var t: float = s2.global_position.y - ssize.y * 0.5
			if t < min_top:
				min_top = t
				top_block = s2
	if top_block != null:
		top_y = min_top
		last_placed = top_block
	else:
		top_y = stable_top_y
		last_placed = stable_last_placed if is_instance_valid(stable_last_placed) else null
	_dbg_cont = "CONT n=%d top_y=%.0f sTop=%.0f" % [nrestored, top_y, stable_top_y]
	current_stone = null
	loading_lb = false
	ad_pending = false
	carrier_speed_mult = 1.0
...
func _update_airport() -> void:
	...
	for p in airport_planes:
		...
		if p.get("kind", "") == "sky":
			p["x"] += p["vx"] * dt
			p["y"] += p["vy"] * dt
			if p["y"] > p["halfh"] + 40.0 or p["y"] < p["top"] - 100.0 or absf(p["x"]) > p["hx"] + 70.0:
...
func _airport_sky_respawn(p: Dictionary) -> void:
	...
	# Появляется ИЗ-ЗА рамки (клиппинг прячет вход): посадка — сверху из-за верхней рамки
	# вниз-наискось, исчезает за перроном; взлёт — снизу из-за перрона вверх-наискось,
	# уходит за верхнюю рамку. Медленно, по диагонали.
	var dir_x: float = -1.0 if randf() < 0.5 else 1.0   # +1 вправо, -1 влево
	p["vx"] = dir_x * randf_range(34.0, 48.0)
	p["x"] = randf_range(-p["hx"] * 0.42, p["hx"] * 0.42)
	if randf() < 0.55:
		p["y"] = p["top"] - randf_range(30.0, 80.0)     # старт ВЫШЕ окна (за рамкой)
		p["vy"] = randf_range(28.0, 40.0)               # снижается за горизонт
	else:
		p["y"] = p["halfh"] - randf_range(10.0, 40.0)   # старт НИЗКО (за перроном)
		p["vy"] = -randf_range(28.0, 40.0)              # набирает высоту, уходит за рамку
...
func _airport_window() -> Node2D:
	...
	# Клиппинг: самолёты-силуэты (дети sky_rect) видны ТОЛЬКО внутри окна — появляются
	# и исчезают за рамкой, не налезая на стену-бэкграунд.
	airport_sky_rect.clip_children = CanvasItem.CLIP_CHILDREN_AND_DRAW
	...
	# Самолёты-силуэты В НЕБЕ: одноцветные, под цвет неба (чуть темнее). Дети sky_rect →
	# клиппятся окном; ниже горизонта их скрывает перрон/здания. Появляются из-за рамки.
	for s in range(2):
		var sky_pl := _sky_plane(Color("85ABCE"))
		sky_pl.scale = Vector2(0.85, 0.85)
		airport_sky_rect.add_child(sky_pl)
		var sp := {"node": sky_pl, "kind": "sky", "sc": 0.85, "hx": W/2, "top": -H/2, "horizon": apron_y, "halfh": H/2}
		_airport_sky_respawn(sp)
		# при старте раскидать по высоте в небе, чтобы не двигались синхронно
		sp["y"] = randf_range(-H/2 + 30, apron_y - 70)
	...
	var lim := W/2 - 90.0
	var taxipl := _airport_plane("E3E8EE", 0.62, true, "C0392B")   # красная ливрея
	...
	var pk_left := _airport_plane("EDEFF2", 1.6, true, "2C5FA6")   # синяя ливрея
	pk_left.position = Vector2(-W/2 + 380, apron_y + 120)
	pk_left.scale.x = 1.6           # носом вправо (к терминалу)
	n.add_child(pk_left)
	var pk_right := _airport_plane("E6EBF0", 1.45, true, "2E8B57")   # зелёная ливрея
...
# Одноцветный силуэт самолёта для дальнего неба (нос вправо).
func _sky_plane(col: Color) -> Node2D:
	var n := Node2D.new()
	var body := Polygon2D.new()
	body.polygon = PackedVector2Array([Vector2(-46, 0), Vector2(-34, -7), Vector2(28, -8), Vector2(46, -3), Vector2(48, 1), Vector2(30, 6), Vector2(-34, 6)])
	body.color = col
	n.add_child(body)
	var wing := Polygon2D.new()
	wing.polygon = PackedVector2Array([Vector2(12, 2), Vector2(-6, 2), Vector2(-26, 20), Vector2(-12, 20)])
	wing.color = col
	n.add_child(wing)
	var tail := Polygon2D.new()
	tail.polygon = PackedVector2Array([Vector2(-40, -6), Vector2(-52, -24), Vector2(-44, -25), Vector2(-32, -7)])
	tail.color = col
	n.add_child(tail)
	return n

func _airport_plane(col: String, sc: float, show_gear: bool = true, accent_hex: String = "2C5FA6") -> Node2D:
	# Пассажирский самолёт сбоку (нос вправо). Крыло и хвост — ярким акцентным цветом
	# и массивные, чтобы хорошо читались.
	var n := Node2D.new()
	n.scale = Vector2(sc, sc)
	var accent := Color(accent_hex)      # ливрея: крылья, киль, полоса (разные у самолётов)
	...
	for i in range(8):
		var w := Polygon2D.new()
		w.polygon = _circle_polygon(3.0, 10); w.position = Vector2(-52 + i * 13, -5)
		w.color = Color("2A3550")
		n.add_child(w)
	var cockpit := Polygon2D.new()
	cockpit.polygon = PackedVector2Array([Vector2(62, -9), Vector2(78, -4), Vector2(62, -1)])
...
func _make_zen_stone(size: Vector2, idx: int) -> Node2D:
	# Если есть PNG-спрайты (пиксель-арт от GPT) — рисуем их под размер блока, по idx.
	var texs := _zen_stone_texs_get()
	if texs.size() > 0:
		var ti: int = (idx if idx >= 0 else 0) % texs.size()
		var tex: Texture2D = texs[ti]
		var nt := Node2D.new()
