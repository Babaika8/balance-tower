...
const BUILD_TAG := "frozen-8"   # ВРЕМЕННО: метка сборки + HUD позиций
...
func _continue_game() -> void:
	...
	for e in stable_snapshot:
		...
		if is_instance_valid(s):
			...
			s.angular_velocity = 0.0
			# STATIC (не KINEMATIC!): у kinematic-заморозки после телепорта коллизия
			# рассинхронизируется — новый блок натыкается на «призрак» вверху и зависает
			# с зазором. STATIC ставит коллизию точно по месту блока (как у земли).
			# Порядок: сперва freeze=true, потом позиция — иначе тело берёт координату
			# из физсервера (место падения).
	...
	_update_score()
	if camera:
		camera.position.y = top_y - 150.0   # сразу кадрируем камеру на башню (как в начале игры)
