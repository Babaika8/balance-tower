...
const BUILD_TAG := "frozen-1"   # ВРЕМЕННО: метка сборки в отладочном трейсе
...
func _continue_game() -> void:
	...
	for e in stable_snapshot:
		...
		if is_instance_valid(s):
			...
			s.angular_velocity = 0.0
			# Восстановленную башню ФИКСИРУЕМ как статичную опору: живые тела на стыках
			# толкаются и рассыпаются, а так последняя устойчивая комбинация держится намертво.
